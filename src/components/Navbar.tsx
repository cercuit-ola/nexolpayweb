...
const Navbar = () => {
  ...
  return (
    <motion.nav
      ...
      className="fixed top-4 left-1/2 -translate-x-1/2 z-50 glass-nav rounded-full px-4 sm:px-6 py-3 flex items-center gap-4 sm:gap-8 w-[95%] sm:w-[90%] max-w-2xl"
    >
      ...
      <div className="hidden md:flex items-center gap-6 ml-auto">
        ...
        <Link to="/dashboard" className="bg-primary text-primary-foreground text-sm font-semibold px-5 py-2 rounded-full hover:opacity-90 transition-opacity">
          Try Now
        </Link>
      </div>
      ...
      {open && (
        ...
        >
          ...
          <Link to="/dashboard" onClick={() => setOpen(false)} className="bg-primary text-primary-foreground text-sm font-semibold px-5 py-2.5 rounded-full text-center hover:opacity-90 transition-opacity">
            Try Now
          </Link>
        </motion.div>
      )}
